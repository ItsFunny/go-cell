...
func wrapCellCommand(cmd *swaggerCommand, config *Config, handler *webdav.Handler) reactor.Function {
	...
	return func(ctx reactor.IBuzzContext, reqData interface{}) error {
		...
		matches := rexp.FindStringSubmatch(req.Request.RequestURI)
		if len(matches) != 3 {
			ctx.Response(reactor.NewContextResponseWrapper(cmd).WithStatus(http.StatusNotFound).WithRet("404 page not found"))
			return nil
		}
		path := matches[2]
		once.Do(func() {
