...
func Test_GetLastInsert(t *testing.T) {
	m := New()
	m.Put(1, 1)
	m.Put(2, 2)
	m.Put(3, 3)
	m.Put(4, 4)
	insert, b := m.GetLastInsert()
	fmt.Println(b)
	fmt.Println(insert)
}

func Test_IteratorWithDelte(t *testing.T) {
	m := New()
	m.Put(1, 1)
	m.Put(2, 2)
	m.Put(3, 3)
...
func TestReverseIterator_remove(t *testing.T) {
	m := New()
	for i := 0; i < 10; i++ {
		m.Put(i, i)
	}
