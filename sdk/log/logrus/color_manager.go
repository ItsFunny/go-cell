...
func SetupDefaultWithInterestModule(modules map[logsdk.Module]Color) {
	g.Do(func() {
...
func SetupDefaultWithInterestModuleOrFields(modules map[logsdk.Module]Color, fields map[string]Color) {
	g.Do(func() {
		for k, v := range modules {
			g.colorCompiledScheme.ModuleColor[k.String()] = getCompiledColor(g.colorStringM[v], "")
		}
...
type ColorScheme struct {
	...
	// 当第一步初始化的时候调用
	ModuleStyle        map[logsdk.Module]Color
...
func newColorManager() *ColorManager {
	...
	colorScheme := &ColorScheme{
		...
		ModuleStyle:        make(map[logsdk.Module]Color, 1),
